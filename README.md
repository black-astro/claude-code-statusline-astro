...
표정은 작업 중에만 번갈아 나오고, 끝나면 첫 번째 표정으로 멈춥니다.
...
**레전드는 글자마다 색이 다릅니다.** 한 글자씩 무지개 색을 입히고, 그 띠가 3초마다
한 칸씩 흘러갑니다. 글자 위로 색이 지나가는 것처럼 보여서 가만히 둬도 반짝입니다.

```
✧（◕ᴗ◕）✧     ← 글자마다 빨강·주황·노랑·초록·하늘·파랑·보라·분홍
              3초마다 색 띠가 한 칸씩 이동
```

표정 4장과 색 이동이 함께 돌아가므로 레전드가 뜬 날은 계속 살아 움직입니다. 다른
등급은 등급 색 하나로 칠해집니다.

> 색이 움직이는 주기는 `ANIM_SECS` (기본 3초)이고, Claude Code가 상태라인을 다시
...
`refreshInterval: 3` 설정으로 3초마다 스크립트가 다시 실행되지만, Claude Code가

...
$KaoTable = @{
    ...
    unique = @(
        ...
        @(  # ✧ヽ（°〇°）ﾉ✧
            @(0x2727, 0x30FD, 0xFF08, 0x00B0, 0x3007, 0x00B0, 0xFF09, 0xFF89, 0x2727),
            @(0x2726, 0x30FE, 0xFF08, 0x00B0, 0x0414, 0x00B0, 0xFF09, 0xFF89, 0x2726)
        ),
        @(  # ✧（ﾉ◕ヮ◕）ﾉ✧
        ...
        @(  # ✧（＠◕ᴗ◕＠）✧
            @(0x2727, 0xFF08, 0xFF20, 0x25D5, 0x1D17, 0x25D5, 0xFF20, 0xFF09, 0x2727),
            @(0x2726, 0xFF08, 0xFF20, 0x25E0, 0x1D17, 0x25E0, 0xFF20, 0xFF09, 0x2726)
        ),
        @(  # ✧（ﾉ≧ڡ≦）ﾉ✧
            @(0x2727, 0xFF08, 0xFF89, 0x2267, 0x06A1, 0x2266, 0xFF09, 0xFF89, 0x2727),
            @(0x2726, 0xFF08, 0x30FD, 0x2267, 0x06A1, 0x2266, 0xFF09, 0x30FD, 0x2726)
        ),
        @(  # ✧＼（◕ᴗ◕）／✧
    ...
    legend = @(
        ...
        @(  # ☆彡（✧ᴗ✧）彡☆
            @(0x2606, 0x5F61, 0xFF08, 0x2727, 0x1D17, 0x2727, 0xFF09, 0x5F61, 0x2606),
            @(0x2605, 0x5F61, 0xFF08, 0x2726, 0x1D17, 0x2726, 0xFF09, 0x5F61, 0x2605),
            @(0x2606, 0x5F61, 0xFF08, 0x2605, 0x1D17, 0x2605, 0xFF09, 0x5F61, 0x2606),
            @(0x2605, 0x5F61, 0xFF08, 0x2606, 0x1D17, 0x2606, 0xFF09, 0x5F61, 0x2605)
        )
    )
    dev = @(
        @(  # ［◉_◉］
        ...
        @(  # （＄_＄）
            @(0xFF08, 0xFF04, 0x005F, 0xFF04, 0xFF09),
            @(0xFF08, 0xFFE5, 0x005F, 0xFFE5, 0xFF09)
        )
    )
}
...
$KaoNames = @{
    common = @('웅크림', '보드람', '멍함', '졸림', '고양이', '히죽', '찡긋', '무표정', '싱글', '순둥', '실눈', '날섬')
    uncommon = @('방긋', '동글', '활짝', '새침', '미소', '초롱', '신남', '윙크', '방실', '함박', '다정', '능글')
    rare = @('반짝', '만세', '별눈', '빛나는미소', '환호', '광채', '손짓', '두근', '별빛', '폭소', '신난만세', '흐뭇')
    unique = @('눈부심', '깜짝', '들뜸', '사랑', '근육', '환희', '볼빨강', '군침', '두손번쩍')
    legend = @('축복', '사랑폭발', '승리', '노래', '별빛세례')
    dev = @('해커', '중괄호', '꺾쇠', '머니')
}

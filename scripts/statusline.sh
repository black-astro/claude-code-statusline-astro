...
# is 36 cells long, so at 1.5 it comes full circle every 24 seconds.
    ...
    printf %s "$1" | awk -v off="$2" -v esc="$ESC" -v rb="$3" '
        BEGIN { n = split(rb, C, " ") }
        {
            for (i = 1; i <= length($0); i++) {
                printf "%s[1;%sm%s", esc, C[((off + i - 1) % n) + 1], substr($0, i, 1)
            }
        }'
}
